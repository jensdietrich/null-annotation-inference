...
if [ ! -d "$RESULT_FOLDER_EXTRACTED_PROPAGATED" ]; then
  echo "Result folder does not exit, creating folder: " $RESULT_FOLDER_EXTRACTED_PROPAGATED
  mkdir -p $RESULT_FOLDER_EXTRACTED_PROPAGATED
else
  echo "Results will be saved in " $RESULT_FOLDER_EXTRACTED_PROPAGATED
fi
...
for module in "${MODULES[@]}" ;do
    ...
    java -jar $PROPAGATOR -p ${PROJECT_FOLDER}/${module} -t gradle_multilang -i $RESULT_FOLDER_EXTRACTED/$NULLABLE-${module}.json -o $RESULT_FOLDER_EXTRACTED_PROPAGATED/$NULLABLE-${module}.json -x $prefix
done
