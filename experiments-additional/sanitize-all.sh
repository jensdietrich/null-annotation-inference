...
cd ${ROOT}
start=`date +%s`
java -Xmx16g -jar $SANITIZER -s -d -m -n -i ${INPUT}/$NULLABLE-guava.json -p ${PROJECT_FOLDER}/guava -t special.guava -o ${OUTPUT}/$NULLABLE-guava.json -sh shaded.json
end=`date +%s`
runtime=$((end-start))
...
cd ${ROOT}
start=`date +%s`
java -Xmx16g -jar $SANITIZER -s -d -m -n -i ${INPUT}/$NULLABLE-error-prone.json -p ${PROJECT_FOLDER}/error-prone -t special.errorprone -o ${OUTPUT}/$NULLABLE-error-prone.json -sh shaded.json
end=`date +%s`
runtime=$((end-start))
