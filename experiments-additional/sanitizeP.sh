...
cd ${ROOT}
start=`date +%s`
java -Xmx16g -jar $SANITIZER -pr -i ${INPUT}/$NULLABLE-guava.json -p ${PROJECT_FOLDER}/guava -t special.guava -o ${OUTPUT}/$NULLABLE-guava.json
end=`date +%s`
runtime=$((end-start))
...
cd ${ROOT}
start=`date +%s`
java -Xmx16g -jar $SANITIZER -pr -i ${INPUT}/$NULLABLE-error-prone.json -p ${PROJECT_FOLDER}/error-prone -t special.errorprone -o ${OUTPUT}/$NULLABLE-error-prone.json
end=`date +%s`
runtime=$((end-start))
